...
class Foundation_Decoder_SAM_BX_FPN(BaseModule):
    ...
    def forward(self, x, batch_data_samples):
        ...
        encoder_feats_a, encoder_feats_b = [], []
        for feat in x:
            encoder_features_1, encoder_features_2 = torch.split(feat, feat.shape[0]//2, dim=0)
            encoder_feats_a.append(encoder_features_1)
            encoder_feats_b.append(encoder_features_2)
        ...
        mask_a = self.output_upscaling(img_a, encoder_feats_a)
        mask_b = self.output_upscaling(img_b, encoder_feats_b)

        b, c, h, w = mask_a.shape
        mask_a_logits = (a_embed @ mask_a.view(b, c, h * w)).view(b, -1, h, w)
        mask_b_logits = (b_embed @ mask_b.view(b, c, h * w)).view(b, -1, h, w)

        logits = torch.cat((mask_a_logits, mask_b_logits, mask_b_logits), dim=1)
    ...
    def loss_by_feat(self, seg_logits: Tensor,
        ...
        if data_type == 'only_cd_label':
            ...
            if self.loss_type == 'FocalLoss':
                ...
                cd_logits = torch.cat((1 - cd_logits, cd_logits), dim=1)
                gt_cd = F.one_hot(gt_cd.long().squeeze(), num_classes=2)
            ...
            if self.train_cfg == 'cp building':
                ...
                loss_dict['loss_cd'] = self.loss_weight[2] * seg_logits.new_zeros([1])
            ...
            if self.loss_type == 'FocalLoss':
                a_logits = torch.cat((1 - a_logits, a_logits), dim=1)
                b_logits = torch.cat((1 - b_logits, b_logits), dim=1)
                cd_logits = torch.cat((1 - cd_logits, cd_logits), dim=1)
                gt_a = F.one_hot(gt_a.long().squeeze(), 2)
                gt_b = F.one_hot(gt_b.long().squeeze(), 2)
                gt_cd = F.one_hot(gt_cd.long().squeeze(), 2)
