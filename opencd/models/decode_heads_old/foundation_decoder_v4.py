...
class Foundation_Decoder_V4(BaseModule):
    ...
    def forward(self, x, batch_data_samples):
        ...
        if x.shape[0] != batch_size:
            ...
            encoder_features = (encoder_features_2 - encoder_features_1).flatten(2).permute(0, 2, 1)
        else:
            encoder_features = x.flatten(2).permute(0,2,1)
        # 拼global和query embed，按token
        global_embed_cd = self.cd_embed.weight
        query_embed = global_embed_cd
        ...
        cd_embed = query_embed
        img_cd = image_embeddings.permute(0, 2, 1).reshape(batch_size, self.channels, self.img_size//self.patch_size, self.img_size//self.patch_size)
