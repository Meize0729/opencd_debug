...
class Foundation_Decoder_SAM_V2(BaseModule):
    ...
    def forward(self, x, batch_data_samples):
        ...
        vit_1, vit_2 = torch.split(x[0], x[0].shape[0]//2, dim=0) 
        vit_low_feature = vit_2 - vit_1

        # 沿batch切开，按token拼起来，[real_bs, token(2048), embed_num]
        encoder_features_1, encoder_features_2 = torch.split(x[-1], x[-1].shape[0]//2, dim=0) 
        vit_high_feature = encoder_features_2 - encoder_features_1
        ...
        mask_cd_decoder = self.output_upscaling(img_cd)
        mask_cd_low = self.compress_vit_feat(vit_low_feature)
        mask_cd_high = self.embedding_encoder(vit_high_feature)

        mask_cd = mask_cd_low + mask_cd_high + mask_cd_decoder
