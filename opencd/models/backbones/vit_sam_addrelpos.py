...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        use_cross_picture_rel_pos: bool = False,
        ...
        self.use_cross_picture_rel_pos = use_cross_picture_rel_pos
        ...
        if self.use_cross_picture_rel_pos:
            self.cross_pic_rel_pos_h = nn.Parameter(
                torch.zeros(2 * input_size[0] - 1, head_embed_dims))
            self.cross_pic_rel_pos_w = nn.Parameter(
                torch.zeros(2 * input_size[1] - 1, head_embed_dims))            

            
    def forward(self, x: torch.Tensor, flag) -> torch.Tensor:

        if flag and x.shape[2] > 20:
            # flag表示数据是不是双图，x.shape[2] > 20表示是不是global
            # 加picture embed
            B, _, _, _ = x.shape
            # 按batch拆双图
            x1, x2 = torch.split(x, B // 2, dim=0)  # Split into two groups
            B, H, W, _ = x1.shape
            # h * w -> token size
            x1 = x1.reshape(B, H*W, -1)
            x2 = x2.reshape(B, H*W, -1)
            # 按token维度拼接 (B, H*W*2, embed)
            x = torch.cat((x1, x2), dim=1)
            # 过qkv linear后reshape
            # (B, H*W*2, embed) -> (B, H*W*2, embed*3) -> (B, H*W*2, 3, 12, embed/3/12) -> (3, B, head, H*W*2, embed/3/12)
            qkv = self.qkv(x).reshape(B, H*W*2, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
            #q k v = (B*head, H*W*2, embed/3/12)
            q, k, v = qkv.reshape(3, B * self.num_heads, H * W * 2, -1).unbind(0)
            attn = (q * self.scale) @ k.transpose(-2, -1)

            if self.use_rel_pos:
                attn[:, :H*W, :H*W] = add_decomposed_rel_pos(attn[:, :H*W, :H*W], q[:, :H*W, :], self.rel_pos_h,
                                          self.rel_pos_w, (H, W), (H, W))
                attn[:, H*W:, H*W:] = add_decomposed_rel_pos(attn[:, H*W:, H*W:], q[:, H*W:, :], self.rel_pos_h,
                                          self.rel_pos_w, (H, W), (H, W))                
            if self.use_cross_picture_rel_pos:
                attn[:, :H*W, H*W:] = add_decomposed_rel_pos(attn[:, :H*W, H*W:], q[:, :H*W, :], self.cross_pic_rel_pos_h,
                                          self.cross_pic_rel_pos_w, (H, W), (H, W))
                attn[:, :H*W, H*W:] = add_decomposed_rel_pos(attn[:, :H*W, H*W:], q[:, H*W:, :], self.cross_pic_rel_pos_h,
                                          self.cross_pic_rel_pos_w, (H, W), (H, W))     
                               
            attn = attn.softmax(dim=-1)
            x = (attn @ v).view(B, self.num_heads, H*W*2,
                            -1).permute(0, 2, 1, 3).reshape(B, H*W*2, -1)
            x = self.proj(x)
            x1, x2 = torch.split(x, H*W, dim=1)
            x = torch.cat((x1, x2), dim=0).reshape(B*2, H, W, -1)
            return x
...
class TransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = Attention(
            ...
            use_cross_picture_rel_pos=True if window_size == 0 else False,
            input_size=input_size if window_size == 0 else
            (window_size, window_size),
            # picture_embed=picture_embed if window_size == 0 else None,
        )
...
class ViTSAM_rel_pos(BaseModule):
    ...
    def __init__(self,
        ...
        self.picture_embed = nn.Parameter(
            torch.zeros(2, 1, 1, self.embed_dims)
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        for i, layer in enumerate(self.layers):
            if i in self.global_attn_indexes:
                x[:B//2] = x[:B//2] + self.picture_embed[0].view(1, 1, 1, -1)
                x[B//2:] = x[B//2:] + self.picture_embed[1].view(1, 1, 1, -1)
            x = layer(x, self.is_two_picture)
            if i in self.out_indices:
                ...
                if self.out_channels > 0 and i == len(self.out_indices) - 1:
                    x_reshape = self.channel_reduction(x_reshape)
...
@MODELS.register_module()
class ViTSAMVisualBackbone_rel_pos(BaseModule):
    def __init__(self,
                 model_cfg,
                 peft_config=None,
                 init_cfg=None,
                 ):
        super(ViTSAMVisualBackbone_rel_pos, self).__init__(init_cfg)
        default_peft_config = {
            "peft_type": "LORA",
            "task_type": "none",
            "inference_mode": False,
